							...
							{#each teams[match.team1.id].players as player, playerIndex}
								<div class="player">
									...
									<p>
										Playing: <input
											type="checkbox"
											bind:checked={match.team1.playing[playerIndex]}
										/>
									</p>
									<p>
									...
									<Select
										...
										add={(index) => {
											...
											for (let i = 0; i < teams[match.team2.id].players.length; i++) {
												console.log(teams[match.team2.id].players[i], index);
												if (teams[match.team2.id].players[i] === index) {
													match.team2.alive[i] = false;
													break;
												}
											}
										}}
							...
							{#each teams[match.team2.id].players as player, playerIndex}
								<div class="player">
									...
									<p>
										Playing: <input
											type="checkbox"
											bind:checked={match.team2.playing[playerIndex]}
										/>
									</p>
									<p>
									...
									<Select
										...
										add={(index) => {
											...
											match.team2.kills = match.team2.kills;
											for (let i = 0; i < teams[match.team1.id].players.length; i++) {
												console.log(teams[match.team1.id].players[i], index);
												if (teams[match.team1.id].players[i] === index) {
													match.team1.alive[i] = false;
													break;
												}
											}
										}}
